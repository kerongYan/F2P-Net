...
class oneConv(nn.Module): ##用在融合模块的一个卷积
    # 卷积+ReLU函数
...
class PromptGenerator(nn.Module):
    ...
    def init_handcrafted(self, x):
        x = self.fft(x, self.freq_nums) # x: N, C, H, W=1*3*1024*1024
    ...
    def get_prompt(self, handcrafted_feature, embedding_feature, hq_feature: torch.Tensor = None):
        ...
        for i in range(self.depth):
            ...
            joint_feature = torch.zeros_like(handcrafted_feature)# Initialise a tensor joint_feature with the same dimensions as handcrafted_feature, to store the merged features.
