...
from F2P_Net.models.encoders import SAMImageEncodeWrapper, SAMPromptEncodeWrapper, ViCNet
from F2P_Net.models.decoders import AFDec
from F2P_Net.models.segment_anything_ext import sam_model_registry

sam_ckpt_path_dict = dict(
    vit_b='/home/lenovo/yankerong/F2P_Net/F2P_Net/pretrained/sam_vit_b_01ec64.pth',
    vit_l='/home/lenovo/yankerong/F2P_Net/F2P_Net/pretrained/sam_vit_l_0b3195.pth',
    vit_h='/home/lenovo/yankerong/F2P_Net/F2P_Net/pretrained/sam_vit_h_4b8939.pth'
)

class BaseF2P_Net(nn.Module):

    def __init__(self, model_type: str):
        super(BaseF2P_Net, self).__init__()
        assert model_type in ['vit_b', 'vit_l', 'vit_h'], f"invalid model_type: {model_type}!"  # 检查model_type是否在['vit_b', 'vit_l', 'vit_h']中
    ...
    def forward(
        ...
        if image_embeddings is None and interm_embeddings is None:
            ...
            ori_img_size = [(imgs[i].shape[-2], imgs[i].shape[-1]) for i in range(len(imgs))]   # Record the original image size
            imgs, point_coords, box_coords = self.preprocess(
                imgs=imgs, point_coords=point_coords, box_coords=box_coords, ori_img_size=ori_img_size)    # Preprocess the images by resizing them to 1024×1024 pixels.
        ...
        masks_sam, masks_hq, images_pre,pixels_pre = self.mask_decoder(
        ...
        postprocess_pixels_pre = [p_pre.clone() for p_pre in pixels_pre]
        for i in range(len(postprocess_pixels_pre)):
            postprocess_pixels_pre[i]= self.postprocess(output_masks=postprocess_pixels_pre[i], ori_img_size=ori_img_size[i])
            # print("postprocess_masks_hq",postprocess_masks_hq[i].shape)   #postprocess_masks_hq torch.Size([2, 1, 528, 622]) 原始图片大小是528*622
        return postprocess_masks_hq, images_pre,postprocess_pixels_pre
    ...
    def infer(
        ...
        masks_pred,images_pre ,pixels_pre= self.forward(
        ...
        return masks_pred,images_pre,pixels_pre
    ...
    def postprocess(output_masks: torch.Tensor, ori_img_size: Tuple):
        ...
        if output_mask_size != ori_img_size:
            ...
            # print("output_masks",output_masks.shape)  #output_masks torch.Size([2, 1, 528, 622])  
        return output_masks
...
class F2P_Net(BaseF2P_Net):
    def __init__(self, model_type: str):
        super(F2P_Net, self).__init__(model_type=model_type)  # model_type: vit_b, vit_l, vit_h  在训练脚本中指定
        self.image_encoder = ViCNet(ori_sam=self.ori_sam, hq_token=self.mask_decoder.hf_token.weight,Auxiliary_encoder_num=18)
