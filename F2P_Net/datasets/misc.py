...
def find_objects_from_mask(
    ...
    if relative_threshold and len(object_indices_all) > 0:
        ...
        # print(f'area_threshold: {area_threshold}, connectivity: {connectivity}')
...
'''
This code primarily processes ground truth segmentation masks (gt_mask) and generates three types of training prompts (point, box, mask). Through steps such as object region extraction, random point selection, bounding box computation, and noise mask generation, the model receives rich training input prompts, aiding it in better learning the object segmentation task.
...
gt_mask: Input ground truth mask, typically a binary segmentation mask of the target object.
tgt_prompts: A list specifying the prompt types to generate, which can be point, box, and mask.
object_connectivity: Object connectivity, defaulting to 8. Determines which pixels belong to the same object (connectivity of 8 implies pixels adjacent horizontally, vertically, and diagonally belong to the same object).
area_threshold: Threshold for object area, used to filter out small objects.
relative_threshold: Whether to employ relative thresholds.
relative_threshold_ratio: Proportion ratio for relative thresholds, determining the maximum area as a percentage of total area.
max_object_num: Maximum permitted number of objects. If multiple objects exist, only up to max_object_num will be selected.
prompt_point_num: Number of random points generated.
ann_scale_factor: Scaling factor for label annotation.
noisy_mask_threshold: Threshold for noise mask, determining noise intensity.
...
def generate_prompts_from_mask(
        ...
        max_object_num: int = 20, prompt_point_num: int = 1,
